...
func _ready():
	...
	new_mat.albedo_color.r = color_curve.sample(fmod(step, 1))
	new_mat.albedo_color.g = color_curve.sample(fmod(step+0.75, 1))
	new_mat.albedo_color.b = color_curve.sample(fmod(step+0.5, 1))
	...
	$Numbers.set_surface_override_material(0, new_mat)
