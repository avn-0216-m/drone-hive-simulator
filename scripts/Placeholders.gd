...
func test(room: Node):
	...
	print("Testing: " + room.name + " at: " + str(room.position))
	
	for cell in room.walls.get_used_cells():
		var converted = local_to_map(map_to_local(Vector3i(cell.x, 0, cell.z)) + room.position)
		if get_cell_item(Vector3i(converted.x, 0, converted.z)) != -1:
...
func add(room: Node):
	...
	for tile in tiles:
		var local_pos = room.walls.map_to_local(Vector3i(tile.x, tile.y, tile.z))
		var world_pos = room.walls.to_global(local_pos)
		var ph_pos = local_to_map(world_pos)
		set_cell_item(Vector3(ph_pos.x, ph_pos.y, ph_pos.z), 0)
